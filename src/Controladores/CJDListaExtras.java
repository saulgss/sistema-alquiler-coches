...
import Vistas.JDListaExtras;
import javax.swing.table.DefaultTableModel;

/**
 * Controlador del JDialog Lista de Extras.
 * Recoge los datos de la vista y los pasa al modelo y viceversa.
 * @author Rafa
 */
public class CJDListaExtras {
    
    private JDListaExtras mostrarExtras;
     ...
     * @param mostrarExtras
     */
    public CJDListaExtras(JDListaExtras mostrarExtras) {
    ...
    public void rellenaTablaExtras() {
        ...
        mostrarExtras.setTMJTableExtras(tablaExtra);
     ...
     */
    public void extraAControlador() {
        int seleccion = mostrarExtras.getFilaSeleccionada();
        if (seleccion >= 0) {

            int id = Integer.parseInt( (String) mostrarExtras.getTMJTableExtras().getValueAt(seleccion, 0) );
            String nombre = (String) mostrarExtras.getTMJTableExtras().getValueAt(seleccion, 1);
            String descripcion = (String) mostrarExtras.getTMJTableExtras().getValueAt(seleccion, 2);
            double precio = Double.parseDouble((String) mostrarExtras.getTMJTableExtras().getValueAt(seleccion, 3) );

            Extras e = new Extras(id, nombre, descripcion, precio);

            ExtrasConsultas.consulta().extraSeleccionado(e);
        }
    }
    ...
    public void buscarExtra(String b) {
        ...
        if (lista != null) {
            ...
            mostrarExtras.muestraExcepcionExtraNulo();
        }
    }
